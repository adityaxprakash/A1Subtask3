...
double alpha ;
vector<double> short_ewma = {0};    //short term exponential moving average 
vector<double> long_ewma = {0};
vector<double> macd = {0};    //moving average convergence divergence
vector<double> signal_ewma={0};    //long term exponential moving average
int curr_stocks = 0;
int curr_position = 0;
...
void dma::calculate_short(){
    alpha=2/(12+1);
    auto start = find(dates.begin(),dates.end(),start_date); 
    int index = distance(dates.begin(), start);
    for (int i = -11 ; i <=0; i++)
    {
        if (i==-11){
            short_ewma.push_back(prices[index+i]);
        }
        else{
            short_ewma.push_back((prices[index+i] *alpha) + (short_ewma.back() * (1-alpha)));
        }
    }
    for (int i = 0; i < distance(start,dates.end()); i++)
    {
        short_ewma.push_back((prices[index+i] *alpha) + (short_ewma.back() * (1-alpha)));
    }
}
void dma::calculate_long(){
    alpha=2/(26+1);
    auto start = find(dates.begin(),dates.end(),start_date); 
    int index = distance(dates.begin(), start);
    for (int i = -11 ; i <=0; i++)
    {
        if (i==-25){
            long_ewma.push_back(prices[index+i]);
        }
        else{
            long_ewma.push_back((prices[index+i] *alpha) + (long_ewma.back() * (1-alpha)));
        }
    }
    for (int i = 0; i < distance(start,dates.end()); i++)
    {
...
void dma::calculate_signal(){
    ...
    {
        if (i==-8){
            signal_ewma.push_back(short_ewma[index+i]-long_ewma[index+i]);
        }
        else{
            signal_ewma.push_back(((short_ewma[index+i]-long_ewma[index+i]) *alpha) + (signal_ewma.back() * (1-alpha)));
        }
    }
    for (int i = 0; i < distance(start,dates.end()); i++)
    {
